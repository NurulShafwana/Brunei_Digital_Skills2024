...
bruneidesign_ind24$variables <- bruneidesign_ind24$variables %>%
  mutate(
    ...
    skill_none   = if_else(Skill == "At least basic level of skills", 1, 0)
  )
...
bruneidesign_ind24$variables$Skill <- factor(bruneidesign_ind24$variables$Skill,
                                       ...
                                       labels =  c('At least basic level of skills',
                                                   'Skills in 2-3 out of 5 areas',
                                                   'At least basic level of skills'
                                                   ),
                                       ordered = T)
#table(bruneidesign_ind24$variables$Skill)
...
a <- tibble(
  labels_cat = substring(names(a), 6),
  mean_num = as.numeric(a),
  mean_lbl = formattable::percent(mean_num, digits = 1)
)


# Generating chart for overall skill indicator
a %>% 
  ggplot(aes(x = '', y = mean_num, fill = labels_cat)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar('y', start = 0) +
  geom_label(aes(label = mean_lbl, group = factor(labels_cat)),
             fill = "white", colour = "black", 
  ...
  scale_fill_manual(
    name = "Skill level",
    values = c(
      "At least basic level of skills" = "#FFB6C1",    
      "Skills in 2-3 out of 5 areas" = "#C7DBFF",  
      "Skills in 4-5 out of 5 areas" = "#B1E5D3"
    )
  ) +
  labs(title = "Overall Skill",
       fill  = 'Skill level')
...
f <- svyby(~Skill,~GEN, bruneidesign_ind24, svymean) %>% 
  ...
  mutate(measure = gsub("Skill\\.", "", measure),
         measure = factor(measure, 
                          levels =  c('At least basic level of skills',
                                      'Skills in 2-3 out of 5 areas',
                                      'Skills in 4-5 out of 5 areas'
...
f %>% 
  mutate(GEN = forcats::fct_reorder(GEN, values_num, .desc = FALSE)) %>% 
  ggplot(aes(y = values_num, fill = GEN, x = measure)) +
  geom_bar(position = 'dodge', stat = 'identity') + 
  geom_label(aes(label = values, group = GEN),
             fill = "white", colour = "black", 
             position = position_dodge(width = .9))+
  scale_y_continuous(labels = scales::percent) +
  labs(title = 'Overall Skill x Genders',
